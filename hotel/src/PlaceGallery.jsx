    ...
    <div className="relative">
      <div className="grid gap-2  grid-cols-[2fr_1fr] rounded-3xl overflow-hidden ">
        <div className="overflow-hidden">
          {place.photos?.[0] && (
              <Image onClick={() => setShowAllPhotos(true)} className=" cursor-pointer object-cover w-full h-full" src={place.photos[0]} alt=""/>
          )}
        </div>
        <div className="grid grid-rows-2 grid-cols-1">
          {place.photos?.[1] && (
            <Image onClick={() => setShowAllPhotos(true)} className=" cursor-pointer object-cover w-full h-full" src={place.photos[1]} alt=""/>
          )}
          {/* <div className="overflow-hidden"> */}
            {place.photos?.[2] && (
